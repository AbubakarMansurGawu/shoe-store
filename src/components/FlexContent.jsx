...
const FlexContent = ({ifExists, endpoint: {heading, title, text, btn, img, url}}) => {
  return (
    <>
     <div className={`flex items-center justify-between lg:flex-col 
     lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' :
      'flex-row'}`}>
        ...
        items-center lg:justify-items-center'>
            ...
            sm:text-2xl font-bold text-slate-900 filter drop-shadow-lg'>
                {title}</h1>
        ...
        lg;max-w-none w-full'>
            ...
            className={`w-auto object-fill transitions-theme
             ${ifExists ? 'h-60 lg:h-56 md:h-h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' :
